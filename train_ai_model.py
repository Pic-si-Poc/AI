...
conn = sqlite3.connect("../Back/database/licenta.db")
cursor = conn.cursor()

# === 2. Citire date din ai_samples ===
query = "SELECT label, ecg_data, emg_data, temp_data, humidity_data FROM ai_samples"
df = pd.read_sql_query(query, conn)

# === 3. Funcție de extragere trăsături ===
...
# === 4. Aplicare + filtrare mostre valide ===
feature_list = []
label_list = []

for _, row in df.iterrows():
    features_row = extract_features(row)
    if features_row and any(val != 0 for val in features_row):
        feature_list.append(features_row)
        label_list.append("sincer" if row['label'] == "control" else row['label'])

# === 5. Transformare în DataFrame ===
...
# === 6. Verificare date suficiente ===
if features.shape[0] < 2:
    print("Prea puține date valide pentru antrenare. Adaugă mai multe mostre.")
    exit()
...
# === 8. Antrenare model cu class_weight ===
model = RandomForestClassifier(n_estimators=200, class_weight='balanced', random_state=42)
model.fit(X_train, y_train)

# === 9. Evaluare și salvare ===
